...
//  LPDIRECT3DINDEXBUFFER9 pIB;
  ...
  DWORD NumPol = pLoadMesh->GetNumFaces();
  //byte SizeOfVertex = pLoadMesh->
  unsigned short *IndexPtr;
  unsigned short bu;
  pLoadMesh->LockIndexBuffer(0,(void**)&IndexPtr);
  for(int i = 0; i < NumPol; i++)
{
bu=IndexPtr[1+i*3];
IndexPtr[1+i*3]=IndexPtr[2+i*3];
IndexPtr[2+i*3]=bu;
//*IndexPtr+=3;
};
  pLoadMesh->UnlockIndexBuffer();
  ...
  *ppMesh = pMesh;
  ...
  if(!(pMesh->NumMaterials = NumMaterials)) {
    ...
    for(DWORD i=0;i<pMesh->NumMaterials;i++) {
      ...
      if(Materials[i].pTextureFilename) {
        char TextureFile[MAX_PATH];
        sprintf(TextureFile, "%s%s", TexturePath,Materials[i].pTextureFilename);
        D3DXCreateTextureFromFile(pDevice,
                                  TextureFile,
                                  &pMesh->pTextures[i]);
      }
